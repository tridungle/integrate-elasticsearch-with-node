...
export default {
  rateLimit: {
    ...
    MAX_CONNECTIONS:
      process.env.MAX_NUMBER_OF_CONNECTIONS_DURING_WINDOW_MS || 10, // start blocking after 10 requests
    MESSAGE:
      process.env.MESSAGE ||
  ...
  logger: {
    ...
    file: {
      ...
      zippedArchive: true,
      maxsize: process.env.LOGGER_LOG_SIZE
        ? Number(process.env.LOGGER_LOG_SIZE)
        : 5242880, // 5MB
      maxFiles: process.env.LOGGER_MAX_FILE
        ? Number(process.env.LOGGER_MAX_FILE)
        : 5
    },
    console: {
      level: "debug",
  ...
  elasticsearch: {
    host: process.env.ELASTIC_SEARCH_HOST || "http://localhost:9200"
  }
};
